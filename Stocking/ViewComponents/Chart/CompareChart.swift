...
struct CompareChart: View {
    ...
    private func normalizedData(for stock: Stock) -> [ChartDataPoint] {
        ...
        guard let baseline = sorted.first?.price, baseline != 0 else {
            return []
        }
        return sorted.map {
            ChartDataPoint(
    ...
    private var combinedData: [SeriesPoint] {
        primaryData.map {
            SeriesPoint(date: $0.date, value: $0.value, series: primary.symbol)
        }
            + secondaryData.map {
                SeriesPoint(
                    date: $0.date,
                    value: $0.value,
                    series: secondary.symbol
                )
            }
    }
    ...
    private var sharedStartDate: Date {
        combinedData.map(\.date).min() ?? .distantPast
    }
    ...
    private func nearestPoint(in data: [ChartDataPoint], to date: Date)
        -> ChartDataPoint?
    {
        data.min {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 4) {
                ForEach(ChartRange.allCases, id: \.self) { range in
                    ...
                    .font(
                        .system(
                            size: 12,
                            weight: selectedRange == range ? .bold : .regular
                        )
                    )
                    .foregroundStyle(
            ...
            .chartForegroundStyleScale([
                primary.symbol: Color.blue, secondary.symbol: Color.yellow,
            ])
            .chartLegend(.hidden)
            .chartXAxis {
                AxisMarks { value in
