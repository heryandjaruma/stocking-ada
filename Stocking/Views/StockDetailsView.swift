...
struct StockDetailsView: View {
    ...
    private var changePercent: Double {
        guard let current = timeRangedStockPriceHistory.last?.price else {
            return 0
        }
        let previous =
            timeRangedStockPriceHistory.last(where: {
                $0.timestamp < currentDate
            })?.price ?? current
        /// default to current if no previous price is found (maybe the case if it's the first day)
        guard previous != 0 else { return 0 }
        return
            (stock.changeForDate(
                currentDate,
                selectedRange.startDate(from: currentDate)
            ) / previous) * 100
    }

    private var priceStatus: PriceStatus {
        let change = stock.changeForDate(
            currentDate,
            selectedRange.startDate(from: currentDate)
        )
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(
                        "$\(timeRangedStockPriceHistory.last?.price ?? 0, specifier: "%.2f")"
                    )
                    ...
                    HStack(spacing: 6) {
                        HStack(spacing: 2) {
                            Image(
                                systemName: priceStatus == .falling
                                    ? "arrow.down"
                                    : priceStatus == .rising
                                        ? "arrow.up" : "minus"
                            )
                            ...
                            Text(
                                "\(abs(stock.changeForDate(currentDate, selectedRange.startDate(from: currentDate))), specifier: "%.2f")"
                            )
